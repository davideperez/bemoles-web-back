...
async function getAllEvents(
  ...
  try {
    ...
    console.error(
      `No se pudieron traer los datos de todos los events desde mongoDB: ${err}`
    );
  }
}
...
async function getEvent(eventId) {
  try {
    const eventPopulated = await eventsDataBase
      .findById(eventId)
      .populate("reserves");
...
async function updateEventByIdInMongoDB(eventId, event) {
  try {
    ...
    console.error(
      `No se pudo actualziar el evento en la base de datos: ${err}`
    );
  }
}
